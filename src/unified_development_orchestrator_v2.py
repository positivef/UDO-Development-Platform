...
class UnifiedDevelopmentOrchestratorV2:
    ...
    def _init_phase_metrics(self) -> Dict[str, PhaseMetrics]:
        ...
        return {
            "ideation": PhaseMetrics(
                ...
                weight_distribution={"user_input": 0.5, "market_data": 0.3, "technical": 0.2},
            ),
            "design": PhaseMetrics(
                phase="design",
                relevant_metrics=["architecture_quality", "scalability", "patterns"],
                required_confidence=0.7,
                evaluation_criteria={"architecture": 0.4, "scalability": 0.3, "maintainability": 0.3},
                weight_distribution={"architecture": 0.5, "patterns": 0.3, "constraints": 0.2},
            ),
            "mvp": PhaseMetrics(
                phase="mvp",
                relevant_metrics=["speed", "core_features", "validation"],
                required_confidence=0.65,
                evaluation_criteria={"core_features": 0.5, "speed": 0.3, "validation": 0.2},
                weight_distribution={"features": 0.6, "time": 0.3, "quality": 0.1},
            ),
            "implementation": PhaseMetrics(
                phase="implementation",
                relevant_metrics=["code_quality", "test_coverage", "performance"],
                required_confidence=0.8,
                evaluation_criteria={"code_quality": 0.3, "test_coverage": 0.3, "performance": 0.2, "security": 0.2},
                weight_distribution={"code_metrics": 0.5, "testing": 0.3, "performance": 0.2},
            ),
        }
        ...
        if learning_file.exists():
            ...
            with lock:
                with open(learning_file, "r", encoding="utf-8") as f:
                    return json.load(f)
        return {"overrides": [], "success_patterns": [], "failure_patterns": [], "phase_performance": {}}
    ...
    def calculate_phase_aware_confidence(self, phase: str, raw_confidence: float, context: Dict) -> float:
        """
        Phase-Aware 신뢰도 계산
        ...
        if phase == "ideation":
            ...
            weighted_confidence = base_confidence * 0.4 + market_factor * 0.3 + feasibility_factor * 0.3
            ...
            weighted_confidence = base_confidence * 0.3 + arch_quality * 0.4 + pattern_match * 0.3
            ...
            weighted_confidence = base_confidence * 0.4 + speed_factor * 0.3 + feature_coverage * 0.3
    ...
    def calculate_weighted_confidence(self, system_confidence: float, uncertainty_confidence: float, phase: str) -> float:
        """
        가중 평균 신뢰도 계산 (곱셈 대신)
        ...
        default_weights = phase_weight_config.get("default", {"system": 0.5, "uncertainty": 0.4, "bonus": 0.1})
        ...
        phase_bonus = self.config.get("phase_bonus", {}).get(phase, self.config.get("phase_bonus", {}).get("default", 0.5))

        # 가중 평균 계산
        weighted_confidence = (
            system_confidence * system_weight + uncertainty_confidence * uncertainty_weight + phase_bonus * bonus_weight
        )
    ...
    def track_uncertainty(self, context: Dict, decision: Dict):
        ...
        if len(self.uncertainty_tracker.confidence_history) > 5:
            ...
            if all(c > 0.7 for c in recent):
                self.uncertainty_tracker.emergent_patterns.append(
                    {"pattern": "high_confidence_streak", "timestamp": datetime.now().isoformat()}
                )

        # 신뢰도 히스토리 추가
        self.uncertainty_tracker.confidence_history.append(decision.get("confidence", 0.5))
        ...
        if self.uncertainty and hasattr(self.uncertainty, "add_observation"):
            ...
            self.uncertainty.add_observation(
                phase=self.context.current_phase, vector=vector, outcome=decision.get("confidence", 0.5) > 0.6
            )

            # 예측 생성
            prediction = self.uncertainty.predict_evolution(vector, self.context.current_phase, hours=24)
    ...
    def start_development_cycle(self, user_request: str) -> Dict:
        ...
        system_recommendation = self._select_optimal_system_v2(user_request, phase_context)

        # Step 2: 개선된 불확실성 평가
        uncertainty_assessment = self._assess_uncertainties_v2(user_request, system_recommendation, phase_context)

        # Step 3: Phase별 AI 협업 패턴
        ai_collaboration = self._determine_ai_collaboration_v2(phase_context, system_recommendation, uncertainty_assessment)

        # Step 4: 개선된 Go/No-Go 결정
        go_decision = self._make_go_decision_v2(system_recommendation, uncertainty_assessment, ai_collaboration, phase_context)

        # Step 5: 실행 계획
        execution_plan = self._create_execution_plan_v2(system_recommendation, ai_collaboration, go_decision, phase_context)

        # 불확실성 추적
        self.track_uncertainty(phase_context, execution_plan)

        # 학습 데이터 업데이트
        self.learning_data["phase_performance"].setdefault(self.context.current_phase, {})
    ...
    def _select_optimal_system_v2(self, request: str, context: Dict) -> Dict:
        ...
        if self.selector:
            # 실제 Selector 사용
            analysis_context = self.selector.analyze_request(request, team_size=context["team_size"], files=context["files"])
            recommendation = self.selector.recommend_system(analysis_context)

            # Phase-aware 조정
            base_confidence = recommendation.confidence
            phase_confidence = self.calculate_phase_aware_confidence(phase, base_confidence, context)
            ...
            # Phase별 기본 신뢰도
            phase_base_confidence = {"ideation": 0.75, "design": 0.7, "mvp": 0.65, "implementation": 0.6, "testing": 0.7}.get(
                phase, 0.5
            )
    ...
    def _assess_uncertainties_v2(self, request: str, system_rec: Dict, context: Dict) -> Dict:
        """개선된 불확실성 평가"""
    ...
    def _determine_ai_collaboration_v2(self, context: Dict, system_rec: Dict, uncertainty: Dict) -> Dict:
        """Phase별 AI 협업 패턴"""
        ...
        return {"pattern": pattern, "ais": ["claude"], "phase_optimized": True}  # 기본

    def _make_go_decision_v2(self, system_rec: Dict, uncertainty: Dict, ai_collab: Dict, context: Dict) -> Dict:
        """개선된 Go/No-Go 결정"""

        phase = context["phase"]

        # 가중 평균 신뢰도 계산 (곱셈 대신)
        confidence = self.calculate_weighted_confidence(system_rec["confidence"], uncertainty["overall_confidence"], phase)

        phase_thresholds = self.config.get("phase_thresholds", {})
        threshold = phase_thresholds.get(phase, phase_thresholds.get("default", 0.6))
    ...
    def execute_plan(self, plan: Dict) -> Dict:
        ...
        if plan["decision"] in ["NEED_MORE_INFO", "NO_GO"]:
            return {"status": "BLOCKED", "reason": "더 많은 정보 필요", "recommendation": plan.get("approach", "RESEARCH")}
    ...
    def record_outcome(self, plan: Dict, execution_result: Dict, user_feedback: Optional[str] = None):
        ...
        try:
            ...
            if phase not in self.learning_data["phase_performance"]:
                self.learning_data["phase_performance"][phase] = {"total": 0, "success": 0, "success_rate": 0.0}
    ...
    def save_state(self, path: Optional[Path] = None) -> Path:
        ...
        target_path = path or _resolve_storage_file(f"udo_state_{self.context.project_name}.json", self.storage_dir)
    ...
    def _create_execution_plan_v2(self, system_rec: Dict, ai_collab: Dict, go_decision: Dict, context: Dict) -> Dict:
        """개선된 실행 계획"""
