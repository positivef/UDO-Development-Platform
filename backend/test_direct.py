...
try:
    ...
    print(
        f"  _mock_service_instance: {type(pcs._mock_service_instance) if pcs._mock_service_instance else 'None'}"
    )
    print(
        f"  _project_context_service: {type(pcs._project_context_service) if pcs._project_context_service else 'None'}"
    )
except Exception as e:
    print(f"  FAILED: {e}")
