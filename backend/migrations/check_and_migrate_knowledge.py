...
def run_migration():
    ...
    try:
        ...
        logger.info(f"[OK] Connected to database: {db_config['database']}")

        # Check existing tables
        existing_tables = check_tables_exist(conn)
        if existing_tables:
            logger.info(f"[WARN] Found existing knowledge tables: {existing_tables}")
            logger.info("Skipping migration (tables already exist)")
            conn.close()
            return

        logger.info("[OK] No knowledge tables found, running migration...")
        ...
        logger.info(f"[OK] Created tables: {new_tables}")
