...
class KnowledgeFeedback(Base):
    ...
    # Document reference
    document_id = Column(String(500), nullable=False, index=True,
                        comment="Obsidian document ID or filename")

    # Search context
    search_query = Column(Text, nullable=False,
                         comment="User's original search query")
    session_id = Column(String(100), nullable=True, index=True,
                       comment="Session identifier for tracking")

    # Feedback signals
    is_helpful = Column(Boolean, nullable=False,
                       comment="Explicit feedback: helpful or not")
    implicit_accept = Column(Boolean, nullable=True,
                            comment="Implicit signal (copy/dismiss)")
    reason = Column(Text, nullable=True,
                   comment="Optional reason for negative feedback")
...
class KnowledgeDocumentScore(Base):
    ...
    first_search = Column(DateTime(timezone=True), nullable=True,
                         comment="First search timestamp")
...
class KnowledgeSearchStats(Base):
    ...
    search_time_ms = Column(Float, nullable=False,
                           comment="Total search time in milliseconds")
    tier1_hits = Column(Integer, nullable=False, default=0,
                       comment="Filename pattern matches")
    tier2_hits = Column(Integer, nullable=False, default=0,
                       comment="Frontmatter YAML matches")
    tier3_hits = Column(Integer, nullable=False, default=0,
                       comment="Full-text content matches")
    total_results = Column(Integer, nullable=False, default=0,
                          comment="Total results returned")
