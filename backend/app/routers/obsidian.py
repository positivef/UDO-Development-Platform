...
async def manual_sync(request: ObsidianAutoSyncRequest) -> ObsidianSyncResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to sync to Obsidian: {str(e)}"
        )
...
async def auto_sync_event(request: ObsidianAutoSyncRequest) -> ObsidianSyncResponse:
    ...
    try:
        ...
        if success:
            ...
            return ObsidianSyncResponse(
                success=False,
                message="Auto-sync failed - check logs"
            )
...
async def save_error_resolution(
    ...
    try:
        ...
        if success:
            ...
            return ObsidianSyncResponse(
                success=False,
                message="Failed to save error resolution"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to save error resolution: {str(e)}"
        )
...
async def resolve_error_tier1(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Tier 1 resolution failed: {str(e)}"
        )
...
async def queue_event(request: ObsidianAutoSyncRequest) -> ObsidianSyncResponse:
    ...
    try:
        ...
        if success:
            ...
            return ObsidianSyncResponse(
                success=False,
                message="Failed to queue event - check logs"
            )
...
async def force_flush() -> ObsidianSyncResponse:
    ...
    try:
        ...
        return ObsidianSyncResponse(
            success=True,
            message=f"Flushed {events_flushed} pending event(s)"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Force flush failed: {str(e)}"
        )
