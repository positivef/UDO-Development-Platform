...
def get_backend_port() -> int:
        ...
        try:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                try:
                    ...
                    print(f"[WARN]  Port {port} from environment is in use")
        except ValueError:
            print(f"[WARN]  Invalid port in environment: {env_port}")
    ...
    if port != default_port:
        print(f"ℹ  Using port {port} (default {default_port} was in use)")
