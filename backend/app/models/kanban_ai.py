...
class TaskSuggestionRequest(BaseModel):
    ...
    context: str = Field(
        ...
        min_length=10,
        max_length=2000,
        description="Project context and requirements (10-2000 chars)"
    )
    num_suggestions: int = Field(
        default=3,
        ge=1,
        le=5,
        description="Number of task suggestions to generate (1-5)"
    )
    include_dependencies: bool = Field(
        default=False,
        description="Whether to suggest task dependencies"
    )
...
class TaskSuggestion(BaseModel):
    ...
    priority: str = Field(
        default="medium",
        description="Suggested priority (critical/high/medium/low)"
    )
    estimated_hours: Optional[float] = Field(
        None,
        ge=0.5,
        le=100.0,
        description="Estimated hours to complete (0.5-100)"
    )
    ...
    suggested_dependencies: List[str] = Field(
        default_factory=list,
        description="Suggested prerequisite tasks (by title)"
    )
    constitutional_compliance: Dict[str, bool] = Field(
        default_factory=dict,
        description="P1-P17 constitutional checks"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="Additional AI-generated metadata"
    )
...
class TaskSuggestionApproval(BaseModel):
    ...
    modifications: Optional[Dict[str, Any]] = Field(
        None,
        description="User modifications to suggestion before approval"
    )
    approval_notes: Optional[str] = Field(
        None,
        max_length=500,
        description="Optional notes about approval decision"
    )
...
class TaskSuggestionApprovalResponse(BaseModel):
    ...
    created_task: Optional[Dict[str, Any]] = Field(
        None,
...
class ConstitutionalViolationError(Exception):
    ...
    def __init__(self, violations: List[str]):
        ...
        super().__init__(
            f"Constitutional violations detected: {', '.join(violations)}"
        )
