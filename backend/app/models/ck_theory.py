...
class RICEScore(BaseModel):
    ...
    reach: int = Field(
        ...
        ge=1,
        le=10,
        description="Number of users/scope affected (1-10)"
    )
    impact: int = Field(
        ...,
        ge=1,
        le=10,
        description="Impact level on users/system (1-10)"
    )
    confidence: int = Field(
        ...,
        ge=1,
        le=10,
        description="Confidence in estimates (1-10)"
    )
    effort: int = Field(
        ...,
        ge=1,
        le=10,
        description="Implementation effort required (1-10)"
    )
...
class DesignAlternative(BaseModel):
    ...
    knowledge_basis: List[str] = Field(
        ...,
        min_items=1,
        description="Knowledge elements used"
    )
        ...
        json_schema_extra = {
            "example": {
                ...
                "cons": [
                    ...
                    "Requires OAuth configuration per provider"
                ],
                "risks": [
...
class TradeoffAnalysis(BaseModel):
    ...
    summary: str = Field(
        ...
        min_length=50,
        max_length=1000,
        description="Overall trade-off summary"
    )
    recommendation: str = Field(
        ...,
        min_length=50,
        max_length=1000,
        description="Recommended alternative with detailed reasoning"
    )
    comparison_matrix: Dict[str, Dict[str, Any]] = Field(
        ...,
        description="Feature comparison matrix across alternatives"
    )
    decision_tree: List[str] = Field(
        ...,
        min_items=1,
        description="Decision-making criteria and flow"
    )
...
class CKTheoryRequest(BaseModel):
    ...
    challenge: str = Field(
        ...
        min_length=10,
        max_length=1000,
        description="Design challenge to explore"
    )
    constraints: Optional[Dict[str, Any]] = Field(
        None,
        description="Design constraints (budget, timeline, team size, etc.)"
    )
    ...
    def validate_constraints(cls, v):
        ...
        allowed_keys = {
            'budget', 'team_size', 'timeline', 'complexity',
            'security_requirement', 'performance_requirement',
            'scalability_requirement', 'maintainability_requirement'
        }
...
class CKTheoryResult(BaseModel):
    ...
    alternatives: List[DesignAlternative] = Field(
        ...
        min_items=3,
        max_items=3,
        description="Three design alternatives (A, B, C)"
    )
    tradeoff_analysis: TradeoffAnalysis = Field(
        ...,
        description="Comparative analysis of alternatives"
    )
    ...
    constraints: Optional[Dict[str, Any]] = Field(
        None,
        description="Applied design constraints"
    )
...
class DesignFeedback(BaseModel):
    ...
    alternative_id: Optional[str] = Field(
        None,
        description="Alternative ID (A, B, or C) if specific"
    )
    rating: int = Field(..., ge=1, le=5, description="Rating 1-5")
    comments: Optional[str] = Field(None, max_length=1000)
    selected_alternative: Optional[str] = Field(
        None,
        description="Which alternative was actually implemented"
    )
