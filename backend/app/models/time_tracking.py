...
class TaskSession(BaseModel):
    ...
    pause_duration_seconds: int = Field(0, description="Total pause duration in seconds")

    # Metrics
    duration_seconds: Optional[int] = Field(None, description="Actual task duration in seconds")
    baseline_seconds: int = Field(..., description="Expected manual task duration in seconds")
    time_saved_seconds: Optional[int] = Field(None, description="Time saved vs baseline")
...
class TimeMetrics(BaseModel):
    ...
    period_type: str = Field(..., description="Period type: daily, weekly, monthly, annual")
    ...
    # Analysis
    bottlenecks: Optional[Dict[str, Any]] = Field(None, description="Identified bottlenecks")
    top_time_savers: Optional[Dict[str, Any]] = Field(None, description="Tasks with most time saved")
...
class ROIReport(BaseModel):
    ...
    manual_time_hours: float = Field(..., description="Total manual baseline time in hours")
    actual_time_hours: float = Field(..., description="Total actual time spent in hours")
    time_saved_hours: float = Field(..., description="Total time saved in hours")

    # ROI calculations
    roi_percentage: float = Field(..., description="ROI percentage: (saved/actual) * 100")
    efficiency_gain: float = Field(..., description="Efficiency: (saved/manual) * 100")

    # Projections
    annual_projection_hours: Optional[float] = Field(None, description="Projected annual time saved")
    annual_projection_value: Optional[float] = Field(None, description="Projected annual value saved")
    ...
    # AI performance
    ai_breakdown: Dict[str, Dict[str, Any]] = Field(..., description="Performance by AI model")

    # Phase performance
    phase_breakdown: Dict[str, Dict[str, Any]] = Field(..., description="Performance by phase")

    # Top performers
    top_time_savers: List[Dict[str, Any]] = Field(..., description="Top 5 time-saving task types")
