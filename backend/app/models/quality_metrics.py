...
class PylintMetrics(BaseModel):
    ...
    messages: List[Dict[str, Any]] = Field(
        default_factory=list, description="Sample issues (first 10)"
    )
...
class TestCoverageMetrics(BaseModel):
    ...
    coverage_percentage: float = Field(
        ..., ge=0.0, le=100.0, description="Overall test coverage percentage"
    )
    ...
    success_rate: float = Field(
        ..., ge=0.0, le=100.0, description="Test success rate percentage"
    )
    files_covered: Optional[int] = Field(
        None, description="Number of files with coverage"
    )
    analyzed_at: str = Field(..., description="ISO 8601 timestamp")
    error: Optional[str] = Field(None, description="Error message if analysis failed")
...
class CodeQualityMetrics(BaseModel):
    ...
    typescript: ESLintMetrics = Field(
        ..., description="ESLint metrics for TypeScript code"
    )
...
class QualityMetricsResponse(BaseModel):
    ...
    overall_score: float = Field(
        ..., ge=0.0, le=10.0, description="Overall quality score (weighted average)"
    )
