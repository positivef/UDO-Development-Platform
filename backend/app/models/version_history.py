...
class VersionCommit(BaseModel):
    ...
    files_added: List[str] = Field(default_factory=list, description="List of added files")
    ...
    udo_execution: Optional[Dict[str, Any]] = Field(
        default=None,
        description="UDO execution details that led to this commit"
    )
...
class VersionHistory(BaseModel):
    ...
    # Statistics
    total_contributors: int = Field(default=0, description="Number of unique contributors")
    first_commit_date: Optional[datetime] = Field(default=None, description="Date of first commit")
...
class VersionComparison(BaseModel):
    ...
    # File changes
    files_changed: List[str] = Field(default_factory=list, description="Files that changed")
    ...
    quality_delta: Optional[Dict[str, float]] = Field(
        default=None,
        description="Change in quality metrics (positive = improvement)"
    )

    # Commits in between
    commits_between: List[VersionCommit] = Field(
        default_factory=list,
        description="Commits between from and to"
    )
...
class VersionHistoryQuery(BaseModel):
    ...
    branch: Optional[str] = Field(default=None, description="Branch to query (default: current)")
    limit: int = Field(default=50, description="Maximum number of commits", ge=1, le=500)
    ...
    since: Optional[datetime] = Field(default=None, description="Commits after this date")
    until: Optional[datetime] = Field(default=None, description="Commits before this date")
