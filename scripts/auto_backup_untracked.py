...
def get_repo_root():
    ...
    try:
        ...
        print("[FAIL] Error: Not a Git repository")
        sys.exit(1)
...
def get_untracked_files():
    ...
    try:
        ...
        print(f"[FAIL] Error getting untracked files: {e}")
        return []
...
def create_backup(repo_root, untracked_files):
    ...
    if not untracked_files:
        print("ℹ  No untracked files to backup")
        return None
    ...
    # Backup metadata
    metadata = {"timestamp": timestamp, "datetime": datetime.now().isoformat(), "repo_root": str(repo_root), "files": []}
    ...
    for rel_path in untracked_files:
        ...
        try:
            ...
            # Add to metadata
            metadata["files"].append({"path": rel_path, "size": src.stat().st_size, "modified": src.stat().st_mtime})

            backed_up_count += 1
            print(f"  [OK] {rel_path}")

        except Exception as e:
            print(f"  [WARN]  Failed to backup {rel_path}: {e}")

    # Save metadata
    with open(backup_dir / BACKUP_METADATA_FILE, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)

    print(f"\n[OK] Backed up {backed_up_count} files to: {backup_dir}")
    return backup_dir
...
def cleanup_old_backups():
    ...
    backups = sorted(
        [d for d in BACKUP_ROOT.iterdir() if d.is_dir() and d.name.startswith("backup_")], key=lambda x: x.name, reverse=True
    )
    ...
    if len(backups) > MAX_BACKUPS:
        for old_backup in backups[MAX_BACKUPS:]:
            try:
                ...
                print(f"[WARN]  Failed to remove {old_backup.name}: {e}")
...
def restore_backup(backup_name, repo_root):
    ...
    if not backup_dir.exists():
        print(f"[FAIL] Backup not found: {backup_name}")
        return
    ...
    for file_info in metadata.get("files", []):
        ...
        try:
            ...
            print(f"  [WARN]  Failed to restore {rel_path}: {e}")
