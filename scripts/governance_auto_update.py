...
def generate_compliance_report() -> Dict:
    ...
    all_templates_exist = all(
        (TEMPLATES_DIR / name / ".governance.yaml").exists()
    ...
    for template_name in ["minimal", "standard", "full"]:
        ...
        if template_path.exists():
            ...
            if config.get("version") != GOVERNANCE_VERSION:
                ...
                print(f"  ✅ Updated {template_name} to version {GOVERNANCE_VERSION}")
...
def run_check(verbose: bool = True) -> Tuple[bool, Dict]:
    ...
    if consistency_issues:
        ...
        print("  ✅ All rule files consistent\n")
    ...
    if verbose:
        print(f"📊 Compliance: {compliance['summary']['passed']}/{compliance['summary']['total']} ({compliance['summary']['percentage']}%)")
    ...
    if verbose:
        print("\n" + "=" * 60)
        print(f"  {'✅ All checks passed!' if all_ok else '⚠️ Issues found - review above'}")
        print("=" * 60 + "\n")
